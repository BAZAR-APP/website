...
const BookingWidget: React.FC<BookingWidgetProps> = ({
  ...
  const getDateRestrictions = useMemo(() => {
    if (!selectedPackageType) return { isDateDisabled: () => false, getSuggestedCheckOut: () => addDays(new Date(), 1) }
  ...
  const currentPackage = packageOptions.find((pkg) => pkg.label === selectedPackage) || packageOptions[0]
  ...
  const getDefaultNights = () => {
    switch (selectedPackageType) {
      ...
      case PackageType.FULL_WEEK: return 7
      case PackageType.FULL_MONTH: return 30
      default: return 1
    }
  }
  ...
  const getCurrentPrice = () => {
    switch (selectedPackageType) {
      case PackageType.WEEKEND: return packageInfo.weekendCost || 0
      case PackageType.WEEKDAY: return packageInfo.weekDaysCost || 0
      case PackageType.FULL_WEEK: return packageInfo.fullWeekCost || 0
      case PackageType.FULL_MONTH: return packageInfo.fullMonthCost || 0
      default: return currentPackage?.basePrice || 0
    }
  }

  const nights = calculateNights()
  const currentPrice = getCurrentPrice()
  const total = currentPrice + bookingConfig.refundableDeposit
    ...
    <div className="bg-[#F9FAFB] rounded-2xl">
        ...
        <div className="px-5 py-2">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-xs text-blue-700">
              {selectedPackageType === PackageType.WEEKEND && "Weekend Package: Select Thursday, Friday, or Saturday. Check-out is automatically set for Sunday."}
              {selectedPackageType === PackageType.WEEKDAY && "Weekday Package: Select Friday for check-in. Check-out is automatically set for Wednesday (5 nights)."}
              {selectedPackageType === PackageType.FULL_WEEK && "Full Week Package: Select any date for a 7-night stay. Check-out is automatically set."}
              {selectedPackageType === PackageType.FULL_MONTH && "Full Month Package: Select any date for a 30-night stay. Check-out is automatically set."}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 divide-x divide-[#D1D5DB]">
            <div className="p-3">
              <CustomPopOver
                ...
                triggerChildren={
                  <span className={`cursor-pointer ${!selectedPackageType ? 'opacity-50 pointer-events-none' : ''}`}>
                    <span className="block text-[10px] font-semibold">CHECK-IN</span>
                    <span className="block text-[14px] text-[#9EA0A2]">
                      {selectedDates?.checkIn ? format(new Date(selectedDates.checkIn), 'dd/MM/yyyy') : 'Select package first'}
                    </span>
                  </span>
                }
              >
                <SimpleCalender
                  ...
                  onDateChange={(selectedDate: Date) => {
                    const suggestedCheckOut = getDateRestrictions.getSuggestedCheckOut(selectedDate)
            ...
            <div className="p-3">
              <span className="cursor-default">
                ...
                <span className="block text-[14px] text-[#9EA0A2]">
                  {selectedDates?.checkOut ? format(new Date(selectedDates.checkOut), 'dd/MM/yyyy') : 'Auto-selected'}
                </span>
      ...
      <div className="px-5 pb-5">
        <Link href={`/chalet/${id}/booking`}>
          <Button 
            className={`w-[100%] mb-5 text-white py-2 rounded-lg font-medium ${
              !selectedPackageType || !selectedDates?.checkIn || !selectedDates?.checkOut 
                ? 'opacity-50 cursor-not-allowed' 
                : 'cursor-pointer'
            }`}
            disabled={!selectedPackageType || !selectedDates?.checkIn || !selectedDates?.checkOut}
          >
            Book Now
          </Button>
        </Link>
        ...
        {selectedPackageType && (
          <div className="flex justify-between items-center">
            <span className="text-[16px] font-normal text-[#19191A] flex items-center">
              {currentPrice} {bookingConfig.currency} × {nights} night{nights > 1 ? 's' : ''}
            </span>
            <span className="font-normal text-[16px] text-[#19191A]">
              {currentPrice * nights} {bookingConfig.currency}
            </span>
          </div>
        )}
